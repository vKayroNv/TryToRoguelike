namespace TryToRoguelike.Items
{
    public class TwoHandedSword : Item
    {
        ...
        public TwoHandedSword(int level)
        {
            ...
            _hp = 0;
            _armor = 0;
            _damage = 10 * item_class + Program.GetIntRandom(0, item_class * level);
            _dodge_chance = Program.GetRandom(-.2, .2);
            _block_chance = Program.GetRandom(-.2, .2);
            _crit_chance = Program.GetRandom(0, .5);
            _crit_increaser = item_class * .1d + item_class * Program.GetRandom(-item_class, item_class) / level / 100d;
        }
    }
}
